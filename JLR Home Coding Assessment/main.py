...
# sorting the list with respect to points
def sorting_points(result):
    return result.results[0].points
...
def best_performances():
    ...
    for constructor in constructors_list:  # loop for 2023 constructors
        for season in range(2023, 1949, -1):  # loop for seasons
            try:
                ...
                if len(results) != 0:
                    ...
                    results.sort(key=sorting_points, reverse=True)
                ...
                print("ERROR: ", constructor.title(), "'s data not found for season ", season)
            except ValueError:
                print("ERROR: ", constructor.title(), "'s data not found for season ", season)
